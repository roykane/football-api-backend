...
router.get('/lich-su-vo-dich', (req, res) => {
  ...
  const cardsHtml = leaguesWithData.map(l => {
    ...
    return `<a href="/lich-su-vo-dich/${l.slug}" class="card" style="display:block;text-decoration:none;padding:16px;margin-bottom:0">
      <h3 style="font-size:16px;font-weight:800;color:#0f172a;margin-bottom:6px">🏆 ${escapeHtml(l.name)}</h3>
      <div style="font-size:13px;color:#475569;margin-bottom:4px"><strong>Đương kim:</strong> ${escapeHtml(latest.name)} (${years[0]}/${(years[0]+1).toString().slice(-2)})</div>
      <div style="font-size:12px;color:#64748b">${years.length} mùa · ${years[years.length-1]} - ${years[0]}</div>
    </a>`;
  }).join('');
...
<body>
  ...
  <div class="container">
    ...
    <div class="hero">
      <h1>🏆 Lịch Sử Vô Địch Bóng Đá</h1>
      <div class="meta">Nhà vô địch ${leaguesWithData.length} giải đấu lớn qua các mùa — chọn giải để xem chi tiết.</div>
    </div>
