...
function baseStyles() {
  return `
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .stadium-hero{position:relative;border-radius:12px;overflow:hidden;margin-bottom:16px;min-height:280px;display:flex;align-items:flex-end;background:#0a1628}
    .stadium-hero img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:0.55}
    .stadium-hero .overlay{position:relative;z-index:2;background:linear-gradient(transparent,rgba(10,22,40,0.95));padding:30px 28px;width:100%;color:#fff}
    .stadium-hero h1{font-size:34px;font-weight:900;color:#fbbf24;margin-bottom:6px;text-shadow:0 2px 8px rgba(0,0,0,0.5)}
    .stadium-hero .alias{font-size:14px;color:#cbd5e1;margin-bottom:8px;font-style:italic}
    .stadium-hero .meta{font-size:13px;color:#cbd5e1;display:flex;gap:14px;flex-wrap:wrap}
    .stadium-hero .meta strong{color:#fff}
    @media(max-width:768px){.stadium-hero{min-height:200px}.stadium-hero h1{font-size:24px}}
...
router.get('/san-van-dong/:slug', (req, res) => {
  ...
  const others = stadiums.filter(s => s.slug !== stadium.slug && s.country === stadium.country).slice(0, 5);
  const othersHtml = others.length > 0
    ? others.map(s => `<a class="sidebar-link" href="/san-van-dong/${escapeHtml(s.slug)}">${escapeHtml(s.name)}</a>`).join('')
    : stadiums.filter(s => s.slug !== stadium.slug).slice(0, 5).map(s => `<a class="sidebar-link" href="/san-van-dong/${escapeHtml(s.slug)}">${escapeHtml(s.name)}</a>`).join('');
...
<body>
  ...
  <div class="container">
    ...
    <div class="stadium-hero">
      <img src="${escapeHtml(stadium.image)}" alt="Sân ${escapeHtml(stadium.name)}">
      <div class="overlay">
    ...
    <div class="layout">
      ...
      <aside class="sidebar">
        ...
        <div class="sidebar-card">
          ...
          <a class="sidebar-link" href="/san-van-dong">Tất cả sân vận động</a>
